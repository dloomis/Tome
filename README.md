...
- **`GET /status` identifies the live recording** — while a session is recording or transcribing, the status payload carries a `recording` object with the meeting `subject` and `suggestedFilename`, so a client can show *which* meeting is in progress before offering to start another. Purely additive: an idle response stays `{"state":"idle"}` and older clients ignore the extra field.
