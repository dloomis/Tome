...
- **Wedged-driver isolation** — every blocking CoreAudio/AVAudioEngine call runs on a dedicated serial queue with a 5-second deadline. An audio driver that hangs mid-bind (observed in the field with a mixer's virtual devices) used to freeze the whole app; now the session start fails with an actionable message, further audio work fails fast instead of queueing behind the stuck call, and when the driver finally recovers Tome heals itself — including re-enumerating the Settings device pickers that showed "(unavailable)" while the driver was stuck.
